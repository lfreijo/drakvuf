...
static const char* fe_pt_regs_names_required[][2] =
{
    {"pt_regs", "r15"},
    {"pt_regs", "r14"},
    {"pt_regs", "r13"},
...
static const char* fe_pt_regs_names_optional[][2] =
{
    {"pt_regs", "cs"},
    {"pt_regs", "flags"},
    {"pt_regs", "sp"},
    ...
    , extract_size(config->extract_size)
{
    ...
    if (!drakvuf_get_kernel_struct_members_array_rva(drakvuf, fe_pt_regs_names_required,
            __FE_PT_REGS_REQUIRED, this->regs.data()))
    {
        fprintf(stderr, "[FILEEXTRACTOR-LINUX] Failed to get required pt_regs offsets, fileextractor disabled\n");
        throw -1;
    }

    /* Get optional pt_regs offsets (cs/ss may be in anonymous unions on kernel 6.x+) */
    if (!drakvuf_get_kernel_struct_members_array_rva(drakvuf, fe_pt_regs_names_optional,
            __FE_PT_REGS_MAX - __FE_PT_REGS_REQUIRED, this->regs.data() + __FE_PT_REGS_REQUIRED))
    {
        fprintf(stderr, "[FILEEXTRACTOR-LINUX] Warning: optional pt_regs offsets unavailable (cs/ss)\n");
    }
    ...
    /* Register syscall hook - try do_syscall_64 first (reliable on FRED kernels
     * where x64_sys_call may be called via static_call, bypassing INT3 at entry) */
    syscall_hook = createSyscallHook("do_syscall_64", &linux_fileextractor::syscall_cb, "do_syscall_64");
    if (syscall_hook)
    {
        fprintf(stderr, "[FILEEXTRACTOR-LINUX] Hooked do_syscall_64\n");
    }
    else
    {
        fprintf(stderr, "[FILEEXTRACTOR-LINUX] do_syscall_64 hook failed, trying x64_sys_call\n");
        /* Fallback to x64_sys_call */
        syscall_hook = createSyscallHook("x64_sys_call", &linux_fileextractor::syscall_cb, "x64_sys_call");
        if (syscall_hook)
        {
            fprintf(stderr, "[FILEEXTRACTOR-LINUX] Hooked x64_sys_call\n");
        }
        else
        {
            fprintf(stderr, "[FILEEXTRACTOR-LINUX] No syscall entry point found, fileextractor disabled\n");
            throw -1;
        }
    }
}
